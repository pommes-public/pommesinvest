...
from helper_functions_invest import discount_values
from subroutines_for_data_input_invest import (
    parse_input_sheets, create_buses,
    create_sources, create_demand_response_units, create_demand, create_sinks,
    create_excess_sinks, create_transformers, create_storages,
    existing_transformers_exo_decom, new_transformers_exo, storages_exo,
    renewables_exo,
    load_input_data)
from helper_functions_invest import convert_annual_limit, resample_timeseries
...
def parse_input_data(path_folder_input,
    ...
    if ActivateDemandResponse:
        add_files['sinks_dr_el'] = (
                'sinks_demand_response_el_' + scenario)
...
def nodes_from_excel(path_folder_input,
                     filename_node_data,
                     filename_cost_data,
    ...
    buses_df, excess_df, shortage_df, commodity_sources_df, \
    ...
    WACC_df, cost_timeseries_df \
        = parse_input_sheets(path_folder_input,
                             filename_node_data,
                             filename_cost_data,
                             filename_node_timeseries,
                             filename_min_max_timeseries,
    ...
    node_dict = create_sources(node_dict, commodity_sources_df, fuel_costs_df, fuel_cost_pathway,
    ...
    if ActivateDemandResponse:
        ...
        ts_keys = ['sinks_dr_el_ts', 'sinks_dr_el_ava_pos_ts',
                   'sinks_dr_el_ava_neg_ts', 'sinks_demand_el_ts']

        for key in ts_keys:
            input_data[key] = resample_timeseries(input_data[key],
                                                  freq='48H',
    ...
    node_dict = create_transformers(node_dict, existing_transformers_df,
    ...
    node_dict = create_storages(node_dict,
    ...
    total_exo_decom_capacity_df = exo_com_costs(startyear,
                                                endyear,
    ...
    return (node_dict, existing_storage_labels, new_built_storage_labels,
            total_exo_com_costs_df, total_exo_com_capacity_df, total_exo_decom_capacity_df,
            emissions_limit)


def nodes_from_excel_rh(path_folder_input,
                        filename_node_data,
                        filename_cost_data,
                        filename_node_timeseries,
                        filename_min_max_timeseries,
    ...
    endtime_RH = (timeseries_start +
                  timeslice_length_with_overlap * timeseries_start.freq).strftime("%Y-%m-%d %H:%M:%S")
    ...
    node_dict, new_built_transformer_labels, endo_exo_exist_df = \
        create_transformers(node_dict,
                            existing_transformers_df,
                            new_built_transformers_df,
                            AggregateInput,
                            RollingHorizon,
                            operation_costs_df, ramping_costs_df,
                            investment_costs_df, WACC_df,
                            cost_timeseries_df,
    ...
    node_dict, new_built_storage_labels, endo_exo_exist_stor_df = \
        create_storages(node_dict,
                        existing_storages_df,
                        new_built_storages_df,
                        RollingHorizon,
                        MaxInvest,
    ...
    total_exo_com_costs_df_RH, total_exo_com_capacity_df_RH, \
    total_exo_decom_capacity_df_RH = exo_com_costs_RH(timeseries_start.year,
                                                      RH_endyear,
                                                      counter,
                                                      years_per_timeslice,
                                                      total_exo_com_costs_df_RH,
    ...
    return (node_dict, new_built_transformer_labels, new_built_storage_labels,
            endo_exo_exist_df, endo_exo_exist_stor_df, existing_storage_labels,
            total_exo_com_costs_df_RH, total_exo_com_capacity_df_RH, total_exo_decom_capacity_df_RH,
            emissions_limit)


def exo_com_costs(startyear,
                  endyear,
                  existing_transformers_decom_df,
                  new_transformers_de_com_df,
                  investment_costs_df,
    ...
    existing_transformers_decom_capacity_df = existing_transformers_exo_decom(existing_transformers_decom_df,
                                                                              startyear,
                                                                              endyear)

    new_transformers_exo_com_costs_df, new_transformers_exo_com_capacity_df, \
    new_transformers_exo_decom_capacity_df = new_transformers_exo(new_transformers_de_com_df,
                                                                  investment_costs_df,
                                                                  WACC_df,
                                                                  startyear,
                                                                  endyear,
    ...
    logging.info('Exogenous transformers costs: {:,.0f}'.format(
        new_transformers_exo_com_costs_df.sum().sum()))

    storages_exo_com_costs_df, storages_exo_com_capacity_df, \
    storages_exo_decom_capacity_df = storages_exo(new_built_storages_df,
                                                  storage_turbine_investment_costs_df,
                                                  storage_pump_investment_costs_df,
                                                  storage_investment_costs_df,
                                                  WACC_df,
    ...
    logging.info('Exogenous storages costs: {:,.0f}'.format(
        storages_exo_com_costs_df.sum().sum()))

    renewables_exo_com_costs_df, renewables_exo_com_capacity_df, \
    renewables_exo_decom_capacity_df = renewables_exo(renewables_com_df,
                                                      investment_costs_df,
                                                      WACC_df,
                                                      startyear,
                                                      endyear,
    ...
    logging.info('Exogenous renewables costs: {:,.0f}'.format(
        renewables_exo_com_costs_df.sum().sum()))

    total_exo_com_costs_df = pd.concat([new_transformers_exo_com_costs_df,
                                        storages_exo_com_costs_df,
                                        renewables_exo_com_costs_df], axis=0, sort=False)

    total_exo_com_capacity_df = pd.concat([new_transformers_exo_com_capacity_df,
                                           storages_exo_com_capacity_df,
                                           renewables_exo_com_capacity_df], axis=0, sort=True)

    total_exo_decom_capacity_df = pd.concat([existing_transformers_decom_capacity_df,
                                             new_transformers_exo_decom_capacity_df,
                                             storages_exo_decom_capacity_df,
                                             renewables_exo_decom_capacity_df], axis=0, sort=True)

    return total_exo_com_costs_df, total_exo_com_capacity_df, total_exo_decom_capacity_df
...
def exo_com_costs_RH(startyear,
    ...
    new_transformers_exo_com_costs_df_RH, new_transformers_exo_com_capacity_df_RH, \
    new_transformers_exo_decom_capacity_df_RH = new_transformers_exo(new_transformers_de_com_df,
                                                                     investment_costs_df,
                                                                     WACC_df,
                                                                     startyear=RH_startyear,
                                                                     endyear=RH_endyear,
    ...
    logging.info('Exogenous transformers costs for run {:d}: {:,.0f}'.format(
        counter, new_transformers_exo_com_costs_df_RH.sum().sum()))

    storages_exo_com_costs_df_RH, storages_exo_com_capacity_df_RH, \
    storages_exo_decom_capacity_df_RH = storages_exo(new_built_storages_df,
                                                     storage_turbine_investment_costs_df,
                                                     storage_pump_investment_costs_df,
                                                     storage_investment_costs_df,
                                                     WACC_df,
    ...
    logging.info('Exogenous storages costs for run {:d}: {:,.0f}'.format(
        counter, storages_exo_com_costs_df_RH.sum().sum()))

    renewables_exo_com_costs_df_total_RH, renewables_exo_com_capacity_renewables_df_RH, \
    renewables_exo_decom_capacity_df_RH = renewables_exo(renewables_com_df,
                                                         investment_costs_df, WACC_df,
                                                         startyear=RH_startyear, endyear=RH_endyear,
                                                         IR=IR,
                                                         discount=discount)
    ...
    total_exo_decom_capacity_df_RH_iteration = pd.concat(
        [existing_transformers_decom_capacity_df_RH,
         ...
         renewables_exo_decom_capacity_df_RH], axis=0, sort=True)

    # Combine the results from previous iterations with the one of the current iteration
    total_exo_com_costs_df_RH = pd.concat([total_exo_com_costs_df_RH,
                                           total_exo_com_costs_df_RH_iteration],
                                          axis=1, sort=True)
    total_exo_com_capacity_df_RH = pd.concat([total_exo_com_capacity_df_RH,
                                              total_exo_com_capacity_df_RH_iteration],
                                             axis=1, sort=True)
    total_exo_decom_capacity_df_RH = pd.concat([total_exo_decom_capacity_df_RH,
                                                total_exo_decom_capacity_df_RH_iteration],
                                               axis=1, sort=True)

    return total_exo_com_costs_df_RH, total_exo_com_capacity_df_RH, total_exo_decom_capacity_df_RH
