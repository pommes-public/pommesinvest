    ...
    def update_model_configuration(self, *model_parameters, nolog=False):
        ...
        *model_parameters : dict
        ...
        if hasattr(self, "end_time"):
            setattr(
                self, "start_year", str(pd.to_datetime(self.end_time).year)
            )
...
def determine_timeslices_RH(starttime,
                            endtime,
                            freq,
                            freq_timesteps,
        ...
        if i != amount_of_timeslices - 1:
            for year in range(start, start + myopic_horizon_in_years):
                if not year == start + myopic_horizon_in_years - 1:
                    ...
                    timeslice_length_dict[i] = (myopic_horizon_in_years,
                                                timeslice_length_dict[i] + timeslice_year_dict[year],
                                                timeslice_length_dict[i] + timeslice_year_dict[
                                                    year] + overlap_in_timesteps)
        ...
        # Last iteration
            ...
            for year in range(start, endyear + 1):
                if not year == endyear:
                    ...
                    timeslice_length_dict[i] = (len(range(start, endyear + 1)),
                                                timeslice_length_dict[i] + timeslice_year_dict[year],
                                                timeslice_length_dict[i] + timeslice_year_dict[
                                                    year] + overlap_in_timesteps)
...
def add_further_constrs(om,
    ...
    if ActivateEmissionsLimit:
        solph.constraints.emission_limit(om, flows=emission_flows,
...
def build_simple_model(path_folder_input,
    ...
    (nodes_dict, existing_storage_labels, new_built_storage_labels,
     ...
     emissions_limit) \
        = nodes_from_excel(path_folder_input,
                           filename_node_data,
                           filename_cost_data,
                           filename_node_timeseries,
                           filename_min_max_timeseries,
    ...
    logging.info('Creating a LP optimization model for integrated\n'
    ...
    om = add_further_constrs(om,
                             ActivateEmissionsLimit,
                             ActivateInvestmentBudgetLimit,
                             emissions_limit,
                             investment_budget)

    return om, existing_storage_labels, new_built_storage_labels, \
           total_exo_com_costs_df, total_exo_com_capacity_df, total_exo_decom_capacity_df


def initial_states_RH(om,
    ...
    transformers_init_df = pd.DataFrame(columns=['Existing_Capacity_Transformer', 'Existing_Capacity_endo',
                                                 'endo_cumulated', 'old_exo'], index=new_built_transformer_labels)

    storages_init_df = pd.DataFrame(columns=['Capacity_Last_Timestep', 'Existing_Inflow_Power',
                                             'Existing_Outflow_Power', 'Existing_Capacity_Storage'],
    ...
    for i, t in transformers_init_df.iterrows():
        ...
        try:
            transformers_init_df.loc[i, 'Existing_Capacity_Transformer'] = transformer['scalars'][
                ((i, 'DE_bus_el'), 'invest')]
            transformers_init_df.loc[i, 'Existing_Capacity_endo'] = endo_exo_exist_df.loc[i, 'Existing_Capacity_endo']
    ...
    for i, s in storages_init_df.iterrows():
        ...
        try:
            ...
            storages_init_df.loc[i, 'Capacity_Last_Timestep'] = \
                storage['sequences'][((i, 'None'), 'storage_content')][timeslice_length_wo_overlap_in_timesteps - 1]
            storages_init_df.loc[i, 'Existing_Inflow_Power'] = storage['scalars'][(('DE_bus_el', i), 'invest')]
            storages_init_df.loc[i, 'Existing_Outflow_Power'] = storage['scalars'][((i, 'DE_bus_el'), 'invest')]
            storages_init_df.loc[i, 'Existing_Capacity_Storage'] = storage['scalars'][((i, 'None'), 'invest')]

            storages_init_df.loc[i, 'Existing_Capacity_endo'] = endo_exo_exist_stor_df.loc[i, 'capacity_endo']
            storages_init_df.loc[i, 'Existing_turbine_endo'] = endo_exo_exist_stor_df.loc[i, 'turbine_endo']
            storages_init_df.loc[i, 'Existing_pump_endo'] = endo_exo_exist_stor_df.loc[i, 'pump_endo']

            storages_init_df.loc[i, 'capacity_endo_cumulated'] = \
                storages_init_df.loc[i, 'Existing_Capacity_Storage'] + storages_init_df.loc[i, 'Existing_Capacity_endo']
            storages_init_df.loc[i, 'turbine_endo_cumulated'] = \
                storages_init_df.loc[i, 'Existing_Outflow_Power'] + storages_init_df.loc[i, 'Existing_turbine_endo']
            storages_init_df.loc[i, 'pump_endo_cumulated'] = \
                storages_init_df.loc[i, 'Existing_Inflow_Power'] + storages_init_df.loc[i, 'Existing_Capacity_endo']

            storages_init_df.loc[i, 'old_exo_cap'] = endo_exo_exist_stor_df.loc[i, 'old_exo_cap']
            storages_init_df.loc[i, 'old_exo_turbine'] = endo_exo_exist_stor_df.loc[i, 'old_exo_turbine']
            storages_init_df.loc[i, 'old_exo_pump'] = endo_exo_exist_stor_df.loc[i, 'old_exo_pump']

        except:

            storages_init_df.loc[i, 'Capacity_Last_Timestep'] = \
                storage['sequences'][((i, 'None'), 'storage_content')][timeslice_length_wo_overlap_in_timesteps - 1]
            storages_init_df.loc[i, 'Existing_Inflow_Power'] = storage['scalars'][(('AT_bus_el', i), 'invest')]
            storages_init_df.loc[i, 'Existing_Outflow_Power'] = storage['scalars'][((i, 'AT_bus_el'), 'invest')]
            storages_init_df.loc[i, 'Existing_Capacity_Storage'] = storage['scalars'][((i, 'None'), 'invest')]
...
def build_RH_model(path_folder_input,
    ...
    (node_dict, new_built_transformer_labels, new_built_storage_labels,
     ...
     emissions_limit) \
        = nodes_from_excel_rh(path_folder_input,
                              filename_node_data,
                              filename_cost_data,
                              filename_node_timeseries,
                              filename_min_max_timeseries,
    ...
    timeseries_start = pd.Timestamp(year=RH_endyear + 1, month=1, day=1, hour=0, freq=freq)
    ...
    return om, es, timeseries_start, new_built_transformer_labels, \
...
def solve_RH_model(om,
    ...
    df_rcut = pd.DataFrame(data=electricity_bus['sequences'][0:timeslice_length_wo_overlap_in_timesteps])
    ...
    for i in storage_labels:
        ...
        storage_capacity_results = \
            pd.DataFrame(data=storage_results_dict[i]['sequences'][0:timeslice_length_wo_overlap_in_timesteps][
        ...
        if i in new_built_storage_labels:
            capacity_invest_results = storage_results_dict[i]['scalars'][[((i, 'None'), 'invest')]]
            electricity_bus_scalars = pd.concat([electricity_bus_scalars, capacity_invest_results], axis=0, sort=True)

    results_scalars = pd.concat([results_scalars, electricity_bus_scalars, ], axis=1, sort=True)
    ...
    if not discount:
        ...
        overall_objective += int((meta_results['objective']) / ((1 + IR) ** (RH_startyear - startyear)))

    overall_solution_time += meta_results['solver']['Time']

    return om, results_sequences, results_scalars, \
...
def reconstruct_objective_value(om):
    ...
    for i, o in om.FLOWS:
        ...
        if om.flows[i, o].investment.ep_costs is not None:
            investment_costs += (om.flows[i, o].invest[i, o] *
