...
def csv_to_excel(path_from, path_to = './inputs/', filename_to = 'power_market_input_data', *args):
...
def datetime_index_from_demand_data(path, file):
    ...
    demand_df = pd.read_csv(path+file, sep=';', decimal=',', parse_dates=True, index_col=0)
...
def timesteps_between_timestamps(ts1, ts2, freq):
    """ Calculate the difference in days between two days using simple arithmetics.
    
    Parameters:
    ----------
    ts1: :obj:`pd.Timestamp`
        The first timestamp
    ts2: :obj:`pd.Timestamp`
        The first timestamp
    freq: :obj:`str
    ...
    """
    timesteps_seconds = {'60min': (24, 3600), '15min': (96, 900)}
    
    diff = ts2 - ts1
    
    timestep_diff = diff.days * timesteps_seconds[freq][0] + math.floor(diff.seconds / timesteps_seconds[freq][1])
...
def resample_timeseries(timeseries,
                        freq,
    ...
    if to_offset(freq) > to_offset(original_freq):
        ...
        # do upsampling
        resampled_timeseries = timeseries.resample(freq).interpolate(method=interpolation_rule)
